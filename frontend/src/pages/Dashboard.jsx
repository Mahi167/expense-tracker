...
import { dashboardStyles, trendStyles, chartStyles } from '../assets/dummyStyle.js';
...
const Dashboard = () => {
  ...
  const handleAddTransaction = async () => {
    ...
    const payload = {
      ...
      catagory : newTransaction.category,
    };
    try{
      setLoading(true);
      ...
      console.error("failed to add :" , err?.response || err.message || err);
    }finally{
      setLoading(false);
    }
  }
  ...
  <div className={dashboardStyles.headerContainer}>
    ...
    {/* Time Frame Buttons */}
            ...
            additionalContent={
              <div className={`mt-2 text-xs flex items-center gap-2 ${
                ...
                <span>
                  ...
                  {expenseChange >= 0 ? "income" : "decreae"} from{' '}
       ...
       <div   className={dashboardStyles.pieChartHeight} 
          ...
          {/* 1. Added safe-guard checking to make sure data array actually exists */}
          {financialOverviewData && financialOverviewData.length > 0 ? (
              ...
              <PieChart className={chartStyles?.pieChart || ""}>
                ...
                >
                  {financialOverviewData.map((entry, index) => (
                    <Cell
                      key={`cell-${index}`}
                      fill={COLORS && COLORS.length > 0 ? COLORS[index % COLORS.length] : "#14b8a6"}
                      stroke="#fff"
                      strokeWidth={2}
                    />
                  ))}
                ...
                <Tooltip
                  formatter={(value) => [`$${Math.round(value || 0).toLocaleString()}`, "Amount"]}
            ...
            /* 2. Clear fallback UI if the array data is missing or empty */
