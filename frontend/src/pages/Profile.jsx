...
import { profileStyles } from '../assets/dummyStyle';
import Modal from 'react-modal';
import { Eye, EyeOff, User, X, Lock } from 'lucide-react'; // Added Lock import
...
// PasswordInput component optimized with React.memo
...
const Profile = ({ onUpdateProfile, onLogout }) => {
  ...
  const handleApiRequest = useCallback(async (method, endpoint, data = null) => { // Fixed typo 'getEndPoints' -> 'endpoint'
    ...
    try {
        ...
        const config = {
            method,
            url: `${BASE_URL}${endpoint}`, // Fixed typo 'endpoits' -> 'endpoint'
  ...
  const handleSaveProfile = async () => {
      try {
          ...
          if (data) {
              ...
              toast.success("Profile Updated Successfully"); // Fixed typo 'SuccessFully'
          }
      } catch (error) {
  ...
  const handlePasswordSubmit = async (e) => {
    ...
    try {
        ...
        toast.success("Password Changed Successfully"); // Fixed typo 'Changes SucessFully '
        setShowPasswordModal(false); // Fixed: Close the modal state properly instead of overwriting showPassword with false
  ...
  const closePasswordModal = useCallback(() => {
    if (!loading) {
        setShowPasswordModal(false); // Fixed: Target the modal boolean visibility state
        setPasswordData({ current: "", new: "", confirm: "" });
        setPasswordErrors({});
        setShowPassword({ current: false, new: false, confirm: false });
    }
      ...
      <div className={profileStyles.mainContainer}>
        <div className={profileStyles.header}>
            ...
            <p className={profileStyles.userEmail}>
                {user.email || "Loading..."} {/* Fixed typo 'Loding...' */}
            </p>
        </div>
                ...
                <div className={profileStyles.card}>
                    <div className="flex justify-center items-center mb-6">
                        <h2 className={profileStyles.cardTitle}>
                            ...
                            Personal Information {/* Fixed typo 'Personnal' */}
                        </h2>
                        {!editMode && (
                            <button onClick={() => setEditMode(true)}
                            className={`${profileStyles.editButton}`} disabled={loading}>
                                {loading ? "Loading..." : "Edit"} {/* Fixed typo 'Loding...' */}
                            </button>
                        )}
                    </div>
                {editMode ? (
                    <div className='space-y-4'>
                        <div>
                            ...
                            onChange={handleInputChange} className={profileStyles.input} disabled={loading} /> {/* Fixed typo 'loding' */}
                        </div>
                        <div>
                        ...
                        <div className='flex gap-3 pt-4'>
                            <button onClick={handleSaveProfile} className={profileStyles.buttonPrimary} disabled={loading}> {/* Fixed typo 'loding' */}
